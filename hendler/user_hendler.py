...
from .buttons import reg_text, menu_text, admin_text
...
async def menu(message: Message):
    await message.answer_photo(
        photo="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQxfTyKiKOaifAdMwtibhG3I4czAMK8YQOMJg&s",
        caption=""" 👋 Hush kelibsiz! \n
  Mazali taomlar olamiga sayohat qiling:\n✨ Mazali taomlarimizdan tanlang va pastdagi tugmalardan birini bosib buyurtma bering! 🚀""",
        reply_markup=await food_button(),
    )
...
async def minus_button(call: CallbackQuery):
    ...
    if quantity > 1:
        ...
        await call.message.edit_reply_markup(
            reply_markup=puls_minus_button(food_id, quantity)
        )
...
async def food_back(call: CallbackQuery):
    ...
    await call.message.edit_media(
        media=InputMediaPhoto(
            media="https://media.istockphoto.com/id/1407832840/photo/foods-enhancing-the-risk-of-cancer-junk-food.jpg?s=612x612&w=0&k=20&c=IBXz9XVfsZS-MM-AOW1kGel3WtgIDZpewFpNO2hGTGE=",
            caption="""
...
async def admit_food(call: CallbackQuery):
    ...
    order_button = InlineKeyboardMarkup(
        inline_keyboard=[
            [
                InlineKeyboardButton(text="Cancel", callback_data="cancel_food"),
                InlineKeyboardButton(
                    text="Send", callback_data=f"Send_{food_id}_{quantity}_{food[3]}"
                ),
            ]
        ]
    )
...
async def get_orders(message: Message, state: FSMContext):
    user_id = int(is_register(message.from_user.id)[0])  
    orders = get_order_info(user_id) 
    ...
    for order in orders:
            ...
            text = (
                ...
                f"📌 Holat: {order[3]} \n "
                f"⏰ Sana: {order[4]}\n"
                f"💰 Jami: {int(order[1]) * int(order[2])} so'm"
            )
